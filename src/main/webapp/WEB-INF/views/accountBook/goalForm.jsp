...
<head>
<meta charset="UTF-8">
<link href="${path }/resources/css/graph.css" rel="stylesheet">
...
<body>
    ...
    <%-- 입력 폼 --%>
    <form method="post" action="goalDetailForm">
      <label for="targetAmount">목표 금액:</label>
        <input type="number" name="targetAmount" id="targetAmount" required><br>
    ...
    <%-- 결과 출력 --%>
    <%-- 입력이 있을 경우에만 계산--%>
    <% if (request.getParameter("targetAmount") != null && request.getParameter("endDate") != null) {
        ...
        int dailySave = targetAmount / daysRemain;
        int weeklySave = targetAmount / weeksRemain;
        int monthlySave = targetAmount / (weeksRemain / 4);
    %>
        <h2>목표 금액: <%= targetAmount %> 원</h2>
